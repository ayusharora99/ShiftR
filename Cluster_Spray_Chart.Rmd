...
```
```{r}
custom_clusters_chart = spray_chart(custom_clusters, aes(x = hc_x, y = hc_y)) +
  geom_point()+
  stat_ellipse(aes(group = custom_cluster))
  ggtitle('Custom Clusters Spray Chart')

ggsave("custom_clusters_spray_chart.png")
custom_clusters_chart
```

```{r}
#clusters_chart = spray_chart(college_traditional, aes(x = hc_x, y = hc_y)) +
#  stat_ellipse(data = . %>% filter(traditional_cluster != 0),
#               aes(group = traditional_cluster)) +
#  ggtitle('Clusters Spray Chart')
